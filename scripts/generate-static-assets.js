...
const ASSETS_MODELS_DIR = resolve(ROOT_DIR, 'assets', 'models');
...
async function copyNpcModel(filename) {
  ...
  const publicTargetPath = join(PUBLIC_MODELS_DIR, filename);
  const assetsTargetPath = join(ASSETS_MODELS_DIR, filename);
  ...
  // Check if source file is too small (likely a placeholder)
  const sourceStats = await stat(sourcePath);
  if (sourceStats.size < 1000) { // Less than 1KB
    console.warn(`Skipping ${filename} - file too small (${sourceStats.size} bytes), likely a placeholder`);
    return;
  }

  // Create target directories
  await mkdir(dirname(publicTargetPath), { recursive: true });
  await mkdir(dirname(assetsTargetPath), { recursive: true });

  // Copy to public directory
  if (existsSync(publicTargetPath)) {
    const [current, source] = await Promise.all([readFile(publicTargetPath), readFile(sourcePath)]);
    if (!current.equals(source)) {
      await copyFile(sourcePath, publicTargetPath);
  ...
  if (existsSync(assetsTargetPath)) {
    const [current, source] = await Promise.all([readFile(assetsTargetPath), readFile(sourcePath)]);
    if (!current.equals(source)) {
      await copyFile(sourcePath, assetsTargetPath);
      console.log(`Updated assets/${filename}`);
    }
  } else {
    await copyFile(sourcePath, assetsTargetPath);
    console.log(`Copied to assets/${filename}`);
  }
}
