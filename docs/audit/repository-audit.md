...
## Overview
- Full file size inventory captured in [`docs/audit/file-sizes.csv`](file-sizes.csv) for reference and tooling import.【F:docs/audit/file-sizes.csv†L1-L12732】
...
- The vendored `Tone.js` bundle in `public/assets/vendor/Tone.js` duplicates the npm package and inflates the repo without use.【F:docs/audit/file-sizes.csv†L10573-L10576】【F:docs/audit/file-sizes.csv†L11555-L11555】

### Duplicate or redundant assets
- Character models exist in three separate trees (`assets/models`, `models`, `public/assets/models`).【F:docs/audit/file-sizes.csv†L38-L43】【F:docs/audit/file-sizes.csv†L67-L73】【F:docs/audit/file-sizes.csv†L11522-L11526】  The generated directory alone is ~20 MB (`du`), so dropping tracked copies could recover tens of megabytes.【44647b†L1-L1】
- `data/agora_local.json` and `athens_places.geojson` are duplicated verbatim under both `data/` and `public/data/` even though runtime only needs one copy.【F:docs/audit/file-sizes.csv†L61-L61】【F:docs/audit/file-sizes.csv†L11556-L11557】【F:src/map/agoraLayer.js†L1-L3】
- Sky textures are stored three times (`assets/sky`, `src/sky`, `public/assets/sky`) despite the manual-drop README; e.g., `day.jpg` appears in both asset trees.【F:docs/audit/file-sizes.csv†L48-L48】【F:docs/audit/file-sizes.csv†L11537-L11537】【F:docs/audit/file-sizes.csv†L11656-L11656】【F:assets/sky/README.md†L1-L44】【e205aa†L1-L1】
- Placeholder files (`placeholder.txt`) proliferate across multiple asset folders without purpose.【F:docs/audit/file-sizes.csv†L58-L58】【F:docs/audit/file-sizes.csv†L77-L77】【F:docs/audit/file-sizes.csv†L11528-L11531】

### Unused or orphaned assets
- Ambient track registry expects eight MP3s, but only dawn/day files exist; missing dusk/night/forest/coast/market/night-crickets references will warn at runtime.【F:src/audio/ambient.ts†L11-L19】【F:docs/audit/file-sizes.csv†L11516-L11520】【F:public/assets/audio/README.md†L7-L16】
- `public/assets/audio/footsteps_dirt.mp3` / `footsteps_stone.mp3` do not match the runtime expectation of `footstep_*.mp3`, so the loaders fail to resolve them.【F:src/audio/footsteps.js†L13-L55】【F:docs/audit/file-sizes.csv†L11518-L11519】【F:public/assets/audio/README.md†L12-L13】
- `public/assets/audio/wolf-howl.mp3` is unused anywhere in `src/` (search only hits unrelated encoded data).【F:docs/audit/file-sizes.csv†L11520-L11520】【2049d9†L1-L3】
- `public/assets/props/torch2.glb` is never referenced by code or config.【F:docs/audit/file-sizes.csv†L11530-L11530】【fddcef†L1-L1】
...
- `public/assets/models/character.glb` and the raw copies exceed 9 MB each.【F:docs/audit/file-sizes.csv†L42-L43】【F:docs/audit/file-sizes.csv†L71-L71】【F:docs/audit/file-sizes.csv†L11524-L11524】
- `src/sky/nightSkyTextureData.js` embeds a 1.2 MB base64 texture that could become an external asset for faster diffs.【F:docs/audit/file-sizes.csv†L11660-L11660】
...
### Debug / console noise
- Production entrypoints log heavily (`console.info`, `console.warn`, `console.error`) within `src/main.js`, `src/entry/app.ts`, `src/app/runApp.ts`, and `src/roads/hybridroads.js` debug helpers.【F:src/main.js†L224-L299】【F:src/entry/app.ts†L20-L43】【F:src/app/runApp.ts†L930-L1002】【F:src/roads/hybridroads.js†L95-L126】
...
- `AmbientAPI` exposes track IDs that no longer exist, so mode-to-track mapping silently fails.【F:src/audio/ambient.ts†L11-L19】【F:src/app/runApp.ts†L715-L758】
...
- Duplicate asset roots (`assets/`, `models/`, `public/assets/`) with inconsistent casing (`Adventurer.glb` vs `adventurer.glb`) suggest partial migrations and complicate tooling.【F:docs/audit/file-sizes.csv†L38-L71】
...
- `public/` currently ships generated binaries, placeholder text files, and unused vendor bundles, all of which bloat static deployments and GitHub Pages publishes.【F:docs/audit/file-sizes.csv†L11516-L11555】
...
- Redundant sky/audio assets in `public/` can be trimmed to the minimal set required for the deployed scene.【F:docs/audit/file-sizes.csv†L11516-L11540】
...
3. **Unused audio bundle** – delete `public/assets/audio/wolf-howl.mp3` and align footstep filenames to code expectations.【F:docs/audit/file-sizes.csv†L11518-L11520】【F:src/audio/footsteps.js†L24-L55】
...
6. **Vendored `Tone.js`** – rely on npm dependency instead of bundling the file in `public/assets/vendor`.【F:docs/audit/file-sizes.csv†L10573-L10576】【F:docs/audit/file-sizes.csv†L11555-L11555】

## Refactoring opportunities
- Create a unified asset manifest to eliminate duplicate directories and ensure build scripts write to an ignored output folder only.【F:scripts/generate-static-assets.js†L12-L23】【F:docs/audit/file-sizes.csv†L38-L73】
...
- Strip or gate debug `console.*` calls behind environment checks to keep production logs clean.【F:src/main.js†L224-L299】【F:src/entry/app.ts†L20-L43】
...
5. **Logging hygiene** – Guard console usage with environment flags or remove verbose info/debug logs in production entry points.【F:src/main.js†L224-L299】【F:src/app/runApp.ts†L930-L965】
